    ...
    return `
      <div class="live-order-card">
        <div class="live-order-top">
          <div>
            ...
            <div class="live-order-vendor">
              Vendor: ${order.vendors?.username || 'Unknown vendor'}
            </div>
          </div>
        ...
        <div class="live-order-meta">
          <span>
            <strong>Total:</strong> R${order.total_price ?? 0}
          </span>

          <span>
            <strong>Ordered:</strong> ${formatElapsedTime(order.created_at)}
          </span>
        </div>
...
async function loadTopVendors() {
  ...
  data.forEach(review => {
    ...
    if (!vendorMap[vendorId]) {
      vendorMap[vendorId] = {
        vendor_id: vendorId,
        username: review.vendors?.username || 'Unknown Vendor',
        totalRating: 0,
        reviewCount: 0
      };
    }
