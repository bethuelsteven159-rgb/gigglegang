...
function renderBadges(item) {
  const allergenBadges = normalizeList(item.allergens).map(allergen => {
  ...
  const dietaryBadges = normalizeList(item.dietary_labels).map(label => {
...
function menuItemMatchesOrderItem(menuItem, orderItem) {
  ...
  return (
    (orderId && String(orderId) === String(menuItem.id)) ||
    (
      orderName &&
...
async function loadVendorRatings() {
  ...
  if (error || !data) {
    console.warn('Could not load review ratings:', error);
    return {};
  }
...
function addCartHandlers() {
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', () => {
      const addToCart = cart.addToCart || window.addToCart;

      if (typeof addToCart !== 'function') {
        console.error('addToCart is not available. Check cart.js export/global setup.');
        return;
      }
    ...
    <div class="menu-item">
      ...
      ${item.vendor_rating ? `
        <div class="menu-item-rating" title="Vendor rating">
          ${renderStars(item.vendor_rating)} ${Number(item.vendor_rating).toFixed(1)}
        </div>
      ` : ''}
...
function renderVendors() {
  ...
  container.innerHTML = allVendors.map(vendor => `
    <div class="menu-item vendor-card">
      ...
      <div class="menu-item-description">
        ${vendor.menu_count || 0} available item${Number(vendor.menu_count) === 1 ? '' : 's'}
      </div>
...
function applyFilters() {
  ...
  if (dietaryFilter) {
    filtered = filtered.filter(item => {
      return normalizeList(item.dietary_labels)
        .map(String)
  ...
  if (excludedAllergens.length) {
    filtered = filtered.filter(item => {
      const allergens = normalizeList(item.allergens).map(String);
...
function resetFilters() {
  ...
  const mostOrderedBtn = byId('mostOrderedBtn');

  if (mostOrderedBtn) {
    mostOrderedBtn.classList.remove('active');
  }
...
function restoreCart() {
  ...
  try {
    if (typeof cart.setCart === 'function') {
      cart.setCart(JSON.parse(savedCart));
    }

    if (typeof cart.updateCartDisplay === 'function') {
      cart.updateCartDisplay();
    }
  } catch (error) {
    console.warn('Ignoring invalid saved cart:', error);
  }
}
...
export async function loadStudentMenu() {
  ...
  for (const vendor of vendors) {
    ...
    menuRows.push(...(menu || []).map(item => ({
      ...
      dietary_labels: normalizeList(item.dietary_labels),
      vendor_id: vendor.id,
...
if (cart.addToCart && !window.addToCart) {
  window.addToCart = cart.addToCart;
}

function bootMenuPage() {
  if (byId('menuContainer')) {
    loadStudentMenu().catch(error => {
      console.error('Student menu boot error:', error);
